...
/**
 * Used to create training data for the reranking model that compares A to B asking the question is A better than B
 * This builds training data like this:
 *  * the positive case Good vs Bad = Yes
 *  * the negative mirror case: Bad vs Good = No
 *  * the neg v neg case: Bad vs Bad = No
 *  To keep the classes somewhat balanced we're going to do 1/3 GvB, 1/3 BvG, 1/3 BvB
 * @author Steve Ash
 */
...
new File("../resources/psaur_rerankA_train.txt").withPrintWriter { pw ->
    ...
    inps.everyParallel { InputRecord input ->
      ...
      cans.alignResults.each { alignResult ->
        alignResult.encodings.eachWithIndex { candResult, index ->
          ...
          ArrayList<String> line = makeLine(best, cand, "Y", bestAns.phones, candResult.phones, newTotal, xx, yy)
          ArrayList<String> line2 = makeLine(cand, best, "N", candResult.phones, bestAns.phones, newTotal, xx, yy)
          pw.println(line.join("\t"))
          pw.println(line2.join("\t"))
        }
      }

      // sample totalPairsPerEntry / 2 "pairs" to include
      // need bvb rows
      def elig = (0..<(ans.size())).findAll { it != bestAlignIndex }
      def possPairs = elig.collectMany { a -> elig.findAll { b -> a != b }.collect { b -> [a, b] }}
      Collections.shuffle(possPairs, rand)
      def bvb = possPairs.subList(0, (int)(totalPairsPerEntryToInclude.intValue() / 2))
      bvb.each { p ->
        Encoding a = ans[p[0]]
        Encoding b = ans[p[1]]
        def aa = score(a, wordShape, encToAlign[a], modePhones, uniqueMode, dups, ans, xx)
        def bb = score(b, wordShape, encToAlign[b], modePhones, uniqueMode, dups, ans, xx)

        ArrayList<String> line = makeLine(aa, bb, "N", a.phones, b.phones, newTotal, xx, yy)
        ArrayList<String> line2 = makeLine(bb, aa, "N", b.phones, a.phones, newTotal, xx, yy)
        pw.println(line.join("\t"))
        pw.println(line2.join("\t"))
      }
...
private makeLine(def aa, def bb, String label, List<String> aPhones, List<String> bPhones, int newTotal, String xx,
                 String yy) {
