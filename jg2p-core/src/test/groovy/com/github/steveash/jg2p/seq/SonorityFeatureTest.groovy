 ...
 * @author Steve Ash
 */
class SonorityFeatureTest extends GroovyTestCase {
  ...
  void testSonority() {
    ...
    def pipe2 = new SonorityFeature()
    def result = pipe2.pipe(pipe1.pipe(inst))
    def ts = result.getData() as TokenSequence
    println ts
    assert ts.get(2).getFeatureValue('SONO_O$o$OaC') > 0
    assert ts.get(4).getFeatureValue('SONO_OoO$a$C') > 0
  }
}
